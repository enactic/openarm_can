...
// C++ counterpart of python/examples/test_posforce.py.
...
int main() {
    ...
    std::vector<ControlMode> control_modes = {ControlMode::POS_FORCE};
    ...
    //   dq : speed limit (rad/s)
    //   i  : current limit, per-unit of the motor's max current (0.0-1.0)
    arm.set_callback_mode_all(CallbackMode::STATE);
    arm.get_arm().posforce_control_all({PosForceParam{0.0, 0.0, 0.0}});
    arm.recv_all();
